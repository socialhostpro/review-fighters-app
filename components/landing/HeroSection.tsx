...
const HeroSection: React.FC<HeroSectionProps> = ({ heroData }) => {
  ...
  return (
    <section
        id="hero"
        className="relative text-white py-20 md:py-32 px-6 overflow-hidden min-h-[70vh] md:min-h-[80vh] flex items-center bg-slate-800"
    >
      {/* Content */}
      <div className="container mx-auto relative z-10 text-center">
        ...
        />
        <p className="text-lg md:text-xl text-gray-200 mb-10 max-w-2xl mx-auto animate-fade-in-up" style={{animationDelay: '0.2s'}}>{subHeadline}</p>
        ...
        <div className="mt-12 md:mt-20 max-w-3xl mx-auto animate-fade-in-up" style={{animationDelay: '0.6s'}}>
          <div className="bg-gray-800 rounded-t-lg shadow-2xl p-3 flex items-center space-x-2">
            <span className="w-3 h-3 bg-red-500 rounded-full"></span>
            <span className="w-3 h-3 bg-yellow-500 rounded-full"></span>
            <span className="w-3 h-3 bg-green-500 rounded-full"></span>
            <div className="bg-gray-700 text-gray-300 text-xs rounded px-3 py-1 flex-grow truncate">{browserMockup.url}</div>
          </div>
          <div className="bg-gray-900 rounded-b-lg shadow-2xl p-6 text-left">
